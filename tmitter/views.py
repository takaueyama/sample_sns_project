...
def home_tmeet_content(request):
    ...
    tmeets = Tmeet.objects.filter(
        Q(user__in=login_user_followings, parent=None) | 
        Q(user=request.user, parent=None)
    )
...
def user_page_content(request):
    try:
        ...
        print(e)
    tmeets = Tmeet.objects.filter(user=display_user, parent=None)
    paginator = Paginator(tmeets, LOADING_COUNT)
    page = request.GET.get('page', default=1)
    max_page_number = math.ceil(tmeets.count() / LOADING_COUNT)
...
def follow(request):
    ...
    #  表示ユーザーがフォローしているユーザーの id のリスト
    display_user_followings_id = Connection.objects.filter(follower=display_user).values_list('following', flat=True)
    #  表示ユーザーをフォローしているユーザーの id のリスト
    display_user_followers_id = Connection.objects.filter(following=display_user).values_list('follower', flat=True)
    ...
    # フォロー解除の処理
    elif Connection.objects.filter(follower=login_user, following=display_user).exists():
    # elif login_user.id in display_user_followers_id:
        Connection.objects.filter(follower=login_user, following=display_user).delete()
        context['method'] = 'unfollow'
        # フォローされた通知の削除
        try:
            unfollowed_notification = Notification.objects.filter(
...
def following_list(request, user_name):
    ...
    # 表示ユーザーがフォローしているユーザーのidのリスト
    display_user_followings_ids = Connection.objects.filter(follower=display_user).values_list('following', flat=True)
    # 表示ユーザーがフォローしているユーザーのクエリセット
    display_user_followings = CustomUser.objects.filter(id__in=display_user_followings_ids)

    # ログインユーザーがフォローしているユーザーのidのリスト
    followings_id = Connection.objects.filter(follower=login_user).values_list('following', flat=True)
    # ログインユーザーがフォローしているユーザーのクエリセット
    login_user_followings = CustomUser.objects.filter(id__in=followings_id)
...
def following_list_content(request):
    ...
    # 表示ユーザーがフォローしているユーザーのクエリセット
    display_user_followings = CustomUser.objects.filter(id__in=display_user_followings_ids)
    print(type(display_user_followings))

    # ログインユーザーがフォローしているユーザーのidのリスト
    login_user_followings = Connection.objects.filter(follower=login_user).values_list('following', flat=True)
    #ログインユーザーがフォローしているユーザーのクエリセット
    login_user_followings = CustomUser.objects.filter(id__in=login_user_followings)
...
def follower_list(request, user_name):
    ...
    # 表示ユーザーをフォローしているユーザーのidのリスト
    display_user_followers_ids = Connection.objects.filter(following=display_user).values_list('follower', flat=True)
    # 表示ユーザーをフォローしているユーザーのクエリセット
    display_user_followers = CustomUser.objects.filter(id__in=display_user_followers_ids)

    # ログインユーザーがフォローしているユーザーのidのリスト
    login_user_followings_id = Connection.objects.filter(follower=login_user).values_list('following', flat=True)
    # ログインユーザーがフォローしているユーザーのクエリセット
    login_user_followings = CustomUser.objects.filter(id__in=login_user_followings_id)
